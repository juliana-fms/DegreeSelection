###---------------------------------------------------###
###   File #3: application to Berkeley Growth Study   ###
...
directory<- getwd() ; setwd(dir = paste0(directory, "/"))
...
data_jags$J_i<- as.numeric(table(data_long$id)) # number of meaurements
...
for(m in 5:30){
   ...
   initial_values<- function(){list(tau_e = rexp(n = 1, rate = 1),
                                    ksi = rmvnorm(n = data_jags$n, mean = rep(x = 0, times = data_jags$m), sigma = diag(x = 1, nrow = data_jags$m)),
   ...
   # mcmcplot(mcmcout = output, parms = parameters[-length(parameters)], dir = results_directory, extension = "html",
   #          greek = TRUE)
   ###---------------------------
   ...
   posterior_sample<- as.matrix(as.mcmc(x = output, start = burn_in, end = number_iterations, thin = lag))
   ...
   difference_xi_boys<- sapply(2:data_jags$m, function(k)(posterior_mu_xi_boys[,k] - posterior_mu_xi_boys[,k-1]))
   colnames(difference_xi_boys)<- sapply(2:data_jags$m, function(k)(paste0(k, "-", k-1)))
   difference_xi_girls<- sapply(2:data_jags$m, function(k)(posterior_mu_xi_girls[,k] - posterior_mu_xi_girls[,k-1]))
   colnames(difference_xi_girls)<- sapply(2:data_jags$m, function(k)(paste0(k, "-", k-1)))
   ###-----------------------------
   ...
   change_xi_boys<- t(sapply(1:nrow(posterior_mu_xi_boys),
                             function(l)(sapply(2:(data_jags$m-1),
                                                function(k)(ifelse(sign(difference_xi_boys[l,k] * difference_xi_boys[l,k-1]) < 0, 1, 0))))))
   colnames(change_xi_boys)<- sapply(strsplit(x = colnames(difference_xi_boys), split = "-"), "[[", 2)[-1]
   
   change_xi_girls<- t(sapply(1:nrow(posterior_mu_xi_girls),
                              function(l)(sapply(2:(data_jags$m-1),
                                                 function(k)(ifelse(sign(difference_xi_girls[l,k] * difference_xi_girls[l,k-1]) < 0, 1, 0))))))
   colnames(change_xi_girls)<- sapply(strsplit(x = colnames(difference_xi_girls), split = "-"), "[[", 2)[-1]
   ###-----------------------------
   
   ###-----------------------------
   ### Saving results
   table_change_boys<- rbind(apply(change_xi_boys, 2, sum), nrow(change_xi_boys) - apply(change_xi_boys, 2, sum))
   table_change_girls<- rbind(apply(change_xi_girls, 2, sum), nrow(change_xi_girls) - apply(change_xi_girls, 2, sum))
   write.table(x = table_change_boys, file =  paste0(results_directory, "table_change_point_mean_curve_boys.txt"),
               row.names = c("change", "no change"))
   write.table(x = table_change_girls, file =  paste0(results_directory, "table_change_point_mean_curve_girls.txt"),
               row.names = c("change", "no change"))
   ###-----------------------------
      ...
      for(ii in 1:data_jags$m){
         for(jj in 1:data_jags$m){
            aux_marginal_variance[,ss]<- aux_marginal_variance[,ss] + 
               (data_jags$basis[,ii] * data_jags$basis[,jj] * posterior_sample[ss, paste0("Sigma_ksi[", ii, ",", jj, "]")])  
         }
      }
   }
   ...
   for(ss in 1:nrow(posterior_sample)){
      for(ll in index_boys_long){
         log_likelihood_marg[ll,ss]<- dnorm(x = data_jags$W[ll], mean = data_jags$basis[ll,]%*%posterior_sample[ss,names_mu_xi_boys], 
                                            sd = sqrt(posterior_sigma2_e[ss] + aux_marginal_variance[ll,ss] ), log = TRUE)
      }
      rm(ll)
      for(ll in index_girls_long){
         log_likelihood_marg[ll,ss]<- dnorm(x = data_jags$W[ll], mean = data_jags$basis[ll,]%*%posterior_sample[ss,names_mu_xi_girls], 
                                            sd = sqrt(posterior_sigma2_e[ss] + aux_marginal_variance[ll,ss] ), log = TRUE)
      }
      rm(ll)
   }
   ...
   comparison_measures_marg<- c(dic_marg, -2*lpml_marg, -2*waic_marg) ; names(comparison_measures_marg)<- c("dic", "lpml", "waic")
